...
# Run one dataset per visible GPU.  The pilot runner writes one evidence shard
# per process; shards are merged only after every dataset process has exited.
...
wait_for_batch() {
  ...
  for index in "${!PIDS[@]}"; do
    if ! wait "${PIDS[$index]}"; then
      echo "Dataset failed: ${NAMES[$index]} (see ${LOG_ROOT}/${NAMES[$index]}.log)" >&2
      FAILED=1
    fi
  done
  PIDS=()
