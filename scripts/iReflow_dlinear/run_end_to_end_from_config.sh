...
WANDB_PROJECT="${WANDB_PROJECT:-iReflow-DLinear-E2E}"
...
"${PYTHON_BIN}" - "${REPO_ROOT}" "${CONFIG_PATH}" "${WANDB_PROJECT}" "${ROOT_PATH}" "${CHECKPOINTS}" "${DEVICE}" "${MOVING_AVG}" "${INDIVIDUAL}" "${USE_NORM}" <<'PY'
import json
import os
import shlex
import subprocess
...
moving_avg = str(cfg.get("moving_avg", moving_avg))
individual = str(cfg.get("individual", individual))
use_norm = str(cfg.get("use_norm", use_norm))
...
cmd = [
    sys.executable,
    "-u",
    "./src/experiments/iReflow_DLinear.py",
    "--wandb_project",
    wandb_project,
    "--is_training",
    "2",
